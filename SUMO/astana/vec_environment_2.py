...
class VECEnvironment(gym.Env):
    ...
    def __init__(self, 
                 ...
                 energy_weight=0.5,  # Weight for energy in the reward function
                 seed=None):
        """
        ...
        Args:
            ...
            energy_csv_path: Path to CSV file with energy consumption data
            energy_weight: Weight of energy consumption in the reward function (0.0-1.0)
        ...
        self.observation_space = spaces.Dict({
            ...
            'task_scenario_id': spaces.Box(low=1, high=10, shape=(1,), dtype=np.float32),  # Added scenario ID
    ...
    def _get_observation(self):
        ...
        if self.current_task is None or self.current_bs is None or self.current_vehicle is None:
            ...
            return {
                ...
                'task_scenario_id': np.array([1], dtype=np.float32),  # Default scenario ID
        ...
        return {
            ...
            'task_scenario_id': np.array([self.current_task["scenario_id"]], dtype=np.float32),  # Added scenario ID
    ...
    def step(self, action):
        ...
        if action == self.nodes_per_bs:
            ...
            if woken_node:
                ...
                # Energy penalty for waking up a node
                energy_penalty = 2.0  # Additional energy consumption for wake-up
                energy_consumption = energy_penalty
                # Small negative reward for waking up a node (energy penalty)
                wake_reward = -self.energy_weight * energy_penalty
                reward += wake_reward
            else:
            ...
            if not bs_instance.nodes[action].active:
                ...
                if success:
                    ...
                    energy_consumption = self.energy_model.get_energy_consumption(scenario_id, duration=processing_time)
                    
                    # Add to history
                    self.energy_consumption_history.append(energy_consumption)
                    
                    # Calculate latency reward component
                    latency_weight = 1.0 - self.energy_weight
                    
                    # Normalize processing time between 0 and 1 (assuming max possible is 10s)
                    max_processing_time = task["deadline"]
                    normalized_time = min(1.0, processing_time / max_processing_time)
                    
                    # Latency component: higher reward for lower latency
                    latency_reward = latency_weight * (1.0 - normalized_time)
                    
                    # Energy component: higher reward for lower energy consumption
                    ...
                    # Total reward is weighted sum of latency and energy components
    ...
    def _simulate_and_get_new_task(self, reward_offset=0.0, energy_offset=0.0):
        ...
        if not done:
            ...
            if vehicle_ids:
                ...
                if in_coverage:
                    # Generate a task
                    scenario_id = random.randint(1, 10)  # Random scenario ID from 1 to 10
                    ...
                    task = {
                        ...
                        "scenario_id": scenario_id,  # Added scenario ID
                        "data_size": data_size,
