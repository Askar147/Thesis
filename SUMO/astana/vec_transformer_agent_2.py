...
# Import the energy-aware VEC environment
...
def flatten_observation(obs):
    ...
    Updated for VEC environment 2 with task_scenario_id
    """
    flattened = []
    ...
    flattened.append(obs['task_scenario_id'][0])  # Add the scenario ID from VEC environment 2
...
def train_vec_transformer(env_config):
    ...
    metrics = {
        ...
        'latencies': [],
        'node_usage': [],
        'energy_consumptions': []  # Add energy consumption tracking
    }
    ...
    for episode in range(num_episodes):
        ...
        episode_latencies = []
        episode_node_usage = []
        episode_energy_consumptions = []  # Add energy tracking
        total_energy = 0  # Track total energy per episode
        ...
        if episode % eval_frequency == 0:
            ...
            avg_energy = np.mean(metrics['energy_consumptions'][-eval_frequency:]) if metrics['energy_consumptions'] else 0
...
def evaluate_transformer_agent(agent, env, num_episodes=10):
    ...
    latencies = []
    node_usage = []
    energy_consumptions = []  # Added energy tracking
    ...
    for _ in range(num_episodes):
        ...
        episode_latencies = []
        episode_node_usage = []
        episode_energy_consumptions = []  # Added energy tracking
        ...
        if episode_energy_consumptions:
            energy_consumptions.append(np.mean(episode_energy_consumptions))
...
def save_transformer_training_results(metrics, env_config, save_dir="transformer_results"):
    ...
    # Plot 1: Rewards, Loss, and Energy
    ...
    # Second figure: Node usage and rejection rates
...
def compare_models(dqn_results_dir, transformer_results_dir, save_dir="comparison_results"):
    ...
    # Plot energy consumption
    plt.subplot(2, 3, 5)
    if 'energy_consumptions' in dqn_metrics and 'energy_consumptions' in transformer_metrics:
        plt.plot(dqn_metrics['energy_consumptions'], alpha=0.3, label='DQN')
        plt.plot(transformer_metrics['energy_consumptions'], alpha=0.3, label='Transformer')
        
        # Calculate and plot moving averages
        if len(dqn_metrics['energy_consumptions']) > window_size:
            dqn_moving_avg = np.convolve(dqn_metrics['energy_consumptions'], 
                                       np.ones(window_size)/window_size, 
                                       mode='valid')
            x_avg = np.arange(window_size-1, len(dqn_metrics['energy_consumptions']))
            plt.plot(x_avg, dqn_moving_avg, 'r-', label='DQN Moving Avg', linewidth=2)
            
        if len(transformer_metrics['energy_consumptions']) > window_size:
            transformer_moving_avg = np.convolve(transformer_metrics['energy_consumptions'], 
                                              np.ones(window_size)/window_size, 
                                              mode='valid')
            x_avg = np.arange(window_size-1, len(transformer_metrics['energy_consumptions']))
            plt.plot(x_avg, transformer_moving_avg, 'b-', label='Transformer Moving Avg', linewidth=2)
    ...
    # Plot rejection rates
    plt.subplot(2, 3, 6)
    if 'rejection_rates' in dqn_metrics and 'rejection_rates' in transformer_metrics:
        plt.plot(dqn_metrics['rejection_rates'], alpha=0.3, label='DQN')
        plt.plot(transformer_metrics['rejection_rates'], alpha=0.3, label='Transformer')
        
        # Calculate and plot moving averages
        if len(dqn_metrics['rejection_rates']) > window_size:
            dqn_moving_avg = np.convolve(dqn_metrics['rejection_rates'], 
                                       np.ones(window_size)/window_size, 
                                       mode='valid')
            x_avg = np.arange(window_size-1, len(dqn_metrics['rejection_rates']))
            plt.plot(x_avg, dqn_moving_avg, 'r-', label='DQN Moving Avg', linewidth=2)
            
        if len(transformer_metrics['rejection_rates']) > window_size:
            transformer_moving_avg = np.convolve(transformer_metrics['rejection_rates'], 
                                              np.ones(window_size)/window_size, 
                                              mode='valid')
            x_avg = np.arange(window_size-1, len(transformer_metrics['rejection_rates']))
            plt.plot(x_avg, transformer_moving_avg, 'b-', label='Transformer Moving Avg', linewidth=2)
        
        plt.xlabel('Episode')
        plt.ylabel('Rejection Rate')
        plt.legend()
        plt.title('Task Rejection Rate Comparison')
        plt.grid(True)
    ...
    if 'energy_consumptions' in dqn_metrics and 'energy_consumptions' in transformer_metrics:
        comparison_table['Avg Energy Consumption'] = [
            np.mean(dqn_metrics['energy_consumptions'][-final_window:]),
            np.mean(transformer_metrics['energy_consumptions'][-final_window:])
        ]
    ...
    print(f"{'Model':<12} {'Avg Reward':<15} {'Completion Rate':<20} {'Latency':<15} {'Node Usage':<15} {'Energy':<15}")
    print("-" * 90)
    for i in range(2):
        ...
        if 'Avg Energy Consumption' in comparison_table:
            row += f"{comparison_table['Avg Energy Consumption'][i]:<15.4f}"
        else:
            row += f"{'N/A':<15}"
...
if __name__ == "__main__":
    ...
    env_config = {
        ...
        'max_queue_length': 50,
        'history_length': 10,
        'energy_csv_path': None,  # Set to actual path for energy data
        'energy_weight': 0.5,     # Weight for energy in reward function
        'seed': 42,
        'num_episodes': 1000,
        'max_steps': 300
    }
