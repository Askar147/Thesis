#!/usr/bin/env python3
"""
Enhanced Curriculum Training for FF-DQN
...
BASE_DIR = "ff_dqn_easy_curriculum"
MODELS_DIR = "models"
...
print(f"Starting Enhanced FF-DQN Curriculum Training in {BASE_DIR}")
...
def run_phase(phase_name, phase_dir, cmd):
    ...
    if models:
        # Look specifically for best_model.pth
        best_models = [m for m in models if "best_model.pth" in m]
        if best_models:
            # Sort by modification time (newest first)
            best_models.sort(key=lambda x: os.path.getmtime(x), reverse=True)
            return best_models[0]
        
        # Fall back to any model if no best_model.pth found
...
checkpoint1 = os.path.join(MODELS_DIR, "ff_dqn_phase1.pth")
...
if not phase2_model:
    ...
    checkpoint2 = os.path.join(MODELS_DIR, "ff_dqn_phase2.pth")
...
if phase3_model:
    checkpoint3 = os.path.join(MODELS_DIR, "ff_dqn_phase3.pth")
...
if phase4_model:
    ...
    checkpoint4 = os.path.join(MODELS_DIR, "ff_dqn_phase4_extreme.pth")
...
final_path = os.path.join(MODELS_DIR, "ff_dqn_curriculum_final.pth")
...
stress_test_dir = os.path.join(MODELS_DIR, "ff_dqn_stress_test")
os.makedirs(stress_test_dir, exist_ok=True)

stress_cmd = [
    "python", "high-load-scenario.py",
    "--ff_model", final_path,
    "--te_model", "best_model/best_model_te.pt",  # Assuming this exists
    "--intensity", "high",
    "--episodes", "2",
...
print(f"\nFF-DQN Curriculum Training Complete!")
